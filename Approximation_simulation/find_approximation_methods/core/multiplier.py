...
class Multiplier:
    def __init__(self) -> None:
        ...
        self.adder = {
                         ...
                         f"ca_{i}": Compressor_4_2() for i in range(36) if
                         i not in [0, 1, 13, 14, 16, 18, 21, 24, 27, 2, 15, 20, 31, 33, 35]
                     }
        ...
        self.forward_sequence = {
            ...
            "adders": {i: adder_id for i, adder_id in
                       enumerate(sorted(self.adder.keys(), key=lambda x: int(x.split("_")[1])))}
        }

        # print(
        #     f"generate multiplier: ha[{adder_num['ha']}], fa[{adder_num['fa']}], ca[{adder_num['ca']}], and[{adder_num['and']}]")
        self._build_graph()
    ...
    def write_csv_file(self, file_dir="./mul_step"):
        ...
        register_list = temp_register_list + input_register_list + output_register_list + ["S1", "S2", "temp_store",
                                                                                           "Cin"]
        ...
        for output_value, transfoer_node in self.output_logic_expression.items():
            ...
            False_logic(generation_operation_time, "temp_store")
            Implication(generation_operation_time, temp_value_record_dict[transfoer_node[0]][transfoer_node[1]],
                        "temp_store")
